...
from automation.ena.single_cell_converter import BAM10X
...
class ENASubmission:
    ...
    *Main class for ENA Brokering. It handles all execution options given by user in command line interface*
    ...
    :param validate: A flag for forcing validating fastq files.
    :type validate: bool
    ...
    def __init__(self, accession, combined_mage_tab=False,
                 combined_file_name=None, conan_email=None, validate=False, skip_copy=False,
                 skip_ena_submission=False, add_samples=False, new_alias=None, replace_idf=False,
                 validated_before=False, skip_date=False, reuse_samples=False,
                 other_experiment_dir=None, move=False, neglect_patterns=None, combined_pairs=False,
                 validate_only=False,
                 mixed_pairs=False, no_spots=False, idf=None, sdrf=None, test=False, b10_x=False, force_copy=False):
        ...
        # print 'Skip: ', self.skip_ena_submission
        ...
        self.exp_dir = ''
        r = retrive_expid_username_by_acc(self.accession)
        if r and len(r) > 0:
            self.exp_dir = 'MAGE-TAB_' + str(r[0]['id'])
            self.exp_path = os.path.join(settings.AUTOSUB_DIR, r[0]['login'], self.exp_dir)
        else:
            print colored.red("Couldn't retrieve submission directory. Acc {} doesn't exist in Subs tracking DB.")
            exit(1)
        ...
        if not combined_mage_tab:
            ...
            self.sdrf_file = combined_file_name  # type: str
            self.idf_file = combined_file_name  # type: str
        ...
        self.idf = IDF(idf_path=os.path.join(self.local_tmp, self.idf_file), combined=combined_mage_tab,
                       skip_release_date=skip_date)  # type: IDF
        self.bam_converter = None
        print(self.exp_path)
        if self.b10_x:

            self.bam_converter = BAM10X(self.accession, os.path.join(self.local_tmp, self.sdrf_file))
        ...
        self.sdrf = SdrfCollection(file_path=os.path.join(self.local_tmp, self.sdrf_file), combined=combined_mage_tab,
                                   ...
                                   mixed_pairs=mixed_pairs, b10_x=self.b10_x)  # type: SdrfCollection

        if self.other_experiment_dir:
            self.ena_sdrf = SdrfCollection(file_path=os.path.join(self.local_tmp, self.other_sdrf_name), combined=True)

        self.exp = ENAExperiment(accession=self.accession, sdrf=self.sdrf, idf=self.idf,
    ...
    def extract_sdrf_file_name(self):
        ...
        # print 'sdrf_file_name ', sdrf_file_name
    ...
    def validate_fastq_files(self):
        ...
        if not self.validated_before:
            ...
            execute_command(
                "ssh sra-login 'source /homes/fg_cur/.bashrc;source /etc/profile; source /home/fg_cur/.bash_profile; "
                "echo \$PATH;cd %s;gxa_validate_fastq.sh %s_files_to_check.txt  "
                "%s > validation_out.txt 2>&1'" % (
                    self.ena_full_path, self.accession, memory), 'fg_cur')

            execute_command(
                'scp sra-login:/fire/staging/aexpress/%s/validation_out.txt %s\n' % (
        ...
        for line in lines:
            ...
            if 'error' in line.lower():
                self.rename_back()
                print colored.red(
                    'ERROR: There was an error in validating fastq files. '
                    'Please check the script output in: ' + local_file)
                exit(1)
            if line == '':
                continue
    ...
    def remove_validation_tmp_files(self):
        execute_command("ssh sra-login 'cd %s;rm *.pipe.*;rm ..*.pipe.*;rm gxa_validate_fastq*.err*;rm "
    ...
    def check_rename(self):
        ...
        for row in self.sdrf.rows:
            ...
            if row.data_file != row.new_data_file:
                ...
                # self.ena.send(cmd)
                cmd = 'ssh sra-login "cd %s; mv %s %s"' % (self.ena_full_path, row.data_file, row.new_data_file)
                ...
                if 'No such file or directory' in out:
                    ...
                    check = ' '.join(
                        execute_command(cmd="ssh sra-login 'cd %s;ls;'" % self.ena_full_path, user='fg_cur'))
    ...
    def move_files_to_local_ena_dir(self):
        ...
        for row in self.sdrf.rows:
            ...
            cmd = "ssh sra-login 'cd %s;mv ../%s ./%s;'" % (self.ena_full_path, file_name, new_file_name)
            # if self.ena_optional_dir:
            #     # cmd = 'mv %s ./%s\n' % (os.path.join(self.ena_optional_dir, file_name), new_file_name)
            #     cmd = "ssh sra-login 'cd %s;mv %s ./%s'" % (
            ...
            if 'No such file or directory' in out:
                ...
                check = ' '.join(
                    execute_command(
                        cmd="ssh sra-login 'cd %s;ls;'" % self.ena_full_path,
                        user='fg_cur'
                    )
                )
    ...
    def move_files_back(self):
        for row in self.sdrf.rows:
            ...
            execute_command(
                "ssh sra-login 'mv %s/%s /fire/staging/aexpress/%s'" %
    ...
    def create_ena_exp_dir(self):
        if self.ena_dir == self.accession:
            execute_command("ssh sra-login 'mkdir /fire/staging/aexpress/%s'" % self.accession, 'fg_cur')
        ...
        execute_command(
            "scp %s/unpacked/%s_files_to_check.txt sra-login:/fire/staging/aexpress/%s/%s_files_to_check.txt\n" % (
    ...
    def submit_xmls(self, nodes_only=False, no_sample=False):
        """
        ...
        :param no_sample: flag for not submitting sample.xml
        :type no_sample: bool
        ...
        files = {}
        keys = ['STUDY', 'EXPERIMENT', 'RUN', 'SUBMISSION']
        if self.add_samples or nodes_only:
            keys = ['EXPERIMENT', 'RUN', 'SUBMISSION']
        if not no_sample:
            keys.append('SAMPLE')
        for k in keys:
            files[k] = open(os.path.join(self.local_tmp, '%s_%s.xml' % (self.accession, k.lower())),
                            'rb')
            # files[k] = os.path.join(self.local_tmp, '%s_%s.xml' % (self.accession, k.lower()))
        ...
        #     files_txt.append('-F "{}=@{}"'.format(k, v))
        # submission_command = """ curl -u era-drop-24:lCV82ecg {} "https://www.ebi.ac.uk/ena/submit/drop-box/submit/" """.format(
        #     ' '.join(files_txt))
        #
        # # print submission_command
        # out, err = execute_command(submission_command)
        # # exit()
        #
        # content = out
        # print content
        if '<html>' in content:
            ...
            time.sleep(20)
            self.submit_xmls(nodes_only, no_sample)
            return
        f = open(os.path.join(self.local_tmp, '%s_receipt.xml' % self.accession), 'w')
        f.write(content)
        f.close()
        if 'success="true' not in content:
            print colored.cyan('FAILURE: Failed submission to ENA. Please check the receipt: ' +
                               os.path.join(self.local_tmp, '%s_receipt.xml' % self.accession))
    ...
    def validate_xml(self, extract_num=0, assay_num=0, sample_num=0):
        ...
        tree = Tree.parse(os.path.join(self.local_tmp, '%s_experiment.xml' % self.accession))
        root = tree.getroot()
        ...
        if len(experiments) != extract_num:
            ...
            Please check the file: ''' + os.path.join(self.local_tmp, '%s_experiment.xml' % self.accession))
            exit(1)

        self.check_utf8(f_type='sample')
        tree = Tree.parse(os.path.join(self.local_tmp, '%s_sample.xml' % self.accession, ))
        root = tree.getroot()
        ...
        if len(samples) != sample_num:
            ...
            Please check the file: ''' % (str(len(samples)), str(sample_num)) +
                              os.path.join(self.local_tmp,
                                           '%s_sample.xml' % self.accession))
            exit(1)
        self.check_utf8(f_type='run')
        tree = Tree.parse(os.path.join(self.local_tmp, '%s_run.xml' % self.accession))
        root = tree.getroot()
        ...
        if not self.sdrf.combined_pairs:
            if len(runs) != assay_num:
                ...
                Please check the file: ''' % (str(len(runs)), str(assay_num)) +
                                  os.path.join(self.local_tmp,
                                               '%s_run.xml' % self.accession))
                exit(1)
        f = open(os.path.join(self.local_tmp, '%s_run.xml' % self.accession), 'r')
        runs = f.read()
        f.close()
        files_not_found = []
        for row in self.sdrf.rows:
    ...
    def check_utf8(self, f_type):
        # print(execute_command('dos2unix {file} {file}'.format(file= os.path.join(self.local_tmp, '%s_%s.xml' % (self.accession, f_type)))))
        f_name = os.path.join(self.local_tmp, '%s_%s.xml' % (self.accession, f_type))
        tmp_f_name = os.path.join(self.local_tmp, '%s_%s.tmp.xml' % (self.accession, f_type))
        ...
        if "Windows" in char_set:
            execute_command('mv {f_name} {tmp}'.format(f_name=f_name, tmp=tmp_f_name))
            cmd = "iconv -f CP1251 -t UTF-8 {tmp} >{f_name}".format(
                tmp=tmp_f_name, f_name=f_name)
            print(cmd)
    ...
    def submit_experiment(self, ):
        ...
        # self.check_rename()
        if self.sdrf.combined_pairs or not self.validate:
        ...
        if not self.skip_ena_submission:
            ...
            self.submit_xmls(no_sample=self.exp.samples_num == 0)
        ...
        if study_submitted and not (runs_submitted or samples_submitted or experiments_submitted):
            print colored.red('Study added but not other nodes. Retrying to fix this')
            self.exp.study_accession = ena_acc
            self.exp.generate_xmls(save_dir=self.local_tmp)

            self.submit_xmls(nodes_only=True, no_sample=self.exp.samples_num == 0)
        ...
        # print("Should start")
        if self.b10_x:
            self.bam_converter.after_broker(os.path.join(LOAD_DIR, self.accession))
        # exit()
    ...
    def copy_to_ena_dropbox(self, extra=False):
        ...
        for row in new_rows:
            ...
            if not self.force_copy and exists_remote(host='sra-login', path='/fire/staging/era/upload/Webin-24/' +
                                                                              row.data_file, user='fg_cur'):
                print colored.red('%s already exists in the Dropbox.' % row.data_file)
                # exit(1)
            else:
                print colored.green('Copying: ' + row.data_file)
                execute_command(
                    "ssh sra-login 'cp %s/%s /fire/staging/era/upload/Webin-24/'" % (
    ...
    def get_spot_length(self):
        ...
        if not self.no_spots:
            ...
            # wait_execution(self.ena)
            execute_command(
                "ssh sra-login 'cd %s;rm spots.txt;touch spots.txt'" % self.ena_full_path,
                'fg_cur'
            )
            ...
            if crams:
                ...
                # execute_command(
                #     'ssh sra-login "cd %s;for file in *.cram; do echo -e -n "$file\\t" >>spots.txt; cramtools '
                #     'fastq -I "$file" | head -n2 | tail -n1 | wc -c >>spots.txt; echo ''>>spots.txt; done;"' %
                #     self.ena_full_path,
                #     'fg_cur')
            if sffs:
            ...
            if bams:
                ...
                commands.append('for file in *.bam ; do echo -e -n "$file\\t" >>spots.txt; '
                                'samtools view "./$file"  | head -2 | tail -1 | wc -m >>spots.txt; '
                                'echo ''>>spots.txt; done;')
            ...
            f.write('\n'.join(commands))
            f.close()
            # execute_command('scp  sra-login:')
            # print 'copying spots'
            # self.plantain.send(
            #     'scp sra-login:/fire/staging/aexpress/%s/spots.txt %s\n' % (
            #         self.ena_dir, self.exp_path))
            # wait_execution(self.plantain)
            execute_command(
                'scp %s sra-login:/fire/staging/aexpress/%s/' % (
                    spot_sh, self.ena_dir), 'fg_cur')
            execute_command("ssh sra-login 'cd %s;sh ./spots.sh'" % self.ena_full_path, 'fg_cur')
            execute_command(
                'scp sra-login:/fire/staging/aexpress/%s/spots.txt %s' % (
                    self.ena_dir, self.exp_path), 'fg_cur')
    ...
    def extract_ena_accession_dir(self):
        ...
        # out = '\n'.join(
        #     execute_command(
        #         "ssh sra-login 'find /fire/staging/aexpress/E-MTAB-* -name \"%s-*\" '" % self.accession,
        #         'fg_cur'
        #     )
        # )
        ...
        if self.renamed:
            for row in self.sdrf.rows:
                if row.new_data_file != row.data_file:
                    ...
                    # wait_execution(self.ena)
                    execute_command(
                        "ssh sra-login 'cd %s;mv %s %s'" % (self.ena_full_path, row.new_data_file, row.data_file),
                        'fg_cur'
                    )
...
def parse_arguments():
    ...
    parser.add_argument('-v', '--validate', action='store_true',
                        help='A flag to force validating fastq files.')
    parser.add_argument('-sc', '--skip_copy', action='store_true',
                        help='A flag for skipping copying fastq filesto ENA dropbox.')
    parser.add_argument('-fc', '--force_copy', action='store_true',
                        help='A flag for force copying fastq files to ENA dropbox.')
    ...
    parser.add_argument('-10x', '--bam_10_x', action='store_true',
                        help='''A flag indicating that this is a 10X experiment.''')
...
if __name__ == '__main__':
    ...
    users = {
        'ahmed': ['Ahmed', 'ahmed@ebi.ac.uk'],
        'anjaf': ['Anja', 'anjaf@ebi.ac.uk'],
        'ngeorge': ['Nancy', 'ngeorge@ebi.ac.uk'],
        'sfexova': ['Silvie', 'sfexova@ebi.ac.uk']
    }
    ...
    if args.conan_email:
        user = ['', args.conan_email]
    elif not user:
        print colored.green('Unknown User, using `ahmed@ebi.ac.uk` as conan email')
        user = ['', 'ahmed@ebi.ac.uk']
    else:
        print colored.green('Hello %s, using' % user[0]), colored.red('`%s`' % user[1], bold=True), colored.green('as conan email')
    ena = ENASubmission(accession=args.accession,

                        combined_mage_tab=args.is_combined,
                        ...
                        combined_pairs=args.combined_pairs,
